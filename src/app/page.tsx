...
export default function Home() {
  ...
  const driveLink =
    "https://drive.google.com/file/d/1pSAg1DGfXTZy28U8nMo0hBfuhUA_hNXN/view?usp=sharing";
  ...
  return (
    <section className="flex flex-col-reverse md:flex-row items-center justify-between gap-8 md:gap-12 py-12 px-4 md:px-8">
      <motion.div
        ...
        className="space-y-5 max-w-xl text-center md:text-left"
      >
        <h1 className="text-3xl md:text-5xl font-extrabold text-white">
          Nelima Sultana
        </h1>

        <p className="text-xl md:text-2xl font-medium text-[#F4FBA3]">
          Full Stack Web Developer
        </p>

        <p className="text-base md:text-lg text-gray-300 leading-relaxed">
          As a <span className="text-[#F4FBA3] font-semibold">Full Stack Developer</span>, I build fast,
          scalable, and intuitive web applications using modern technologies like
          <span className="text-[#F4FBA3] font-semibold"> React</span>,
          <span className="text-[#F4FBA3] font-semibold"> Next.js</span>,
          <span className="text-[#F4FBA3] font-semibold"> Tailwind CSS</span>, and
          <span className="text-[#F4FBA3] font-semibold"> Node.js</span>.
        </p>

        <p className="text-base md:text-lg text-gray-300 leading-relaxed">
          I love transforming ideas into reality through code. Whether it’s a startup MVP, a business dashboard,
          or a personal passion project, I strive to deliver products that are not only functional,
          but also visually striking.
        </p>

        <p className="text-base md:text-lg text-gray-300 leading-relaxed">
          Let’s build something great together. Check out my work or get in touch!
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-6 justify-center md:justify-start">
          ...
          <Link
            href="/projects"
            className="border border-[#F4FBA3] text-[#F4FBA3] px-6 py-2 rounded-md font-medium hover:bg-[#F4FBA3] hover:text-black transition text-center"
          >
            View Projects
          </Link>
        </div>
...
function SocialButton({ name, href }: Props) {
  return (
    <Link
      ...
      className="text-white px-4 py-2 border border-white rounded-md hover:bg-[#F4FBA3] hover:text-black transition"
    >
      {name}
    </Link>
  );
